...
**This checks a conditional global reduction, not the whole proof from scratch.** It assumes the 110 explicitly listed restricted tensor lower bounds in `payload.json.gz`. It checks their application to all seven global count trees, both direct pair cases, and exhaustive pair normalization. Their earlier published backtracking proofs and recovered restricted certificates are not included or replayed. `lower_bound_premises_replayed` is always false. A successful output means: if all listed rank premises hold, then rank over F2 of 3×3 matrix multiplication is at least 21.
