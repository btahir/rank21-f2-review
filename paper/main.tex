...
\author{Review draft}
\date{7 September 2026\quad---\quad{}Prepared for independent review}
\begin{document}
\maketitle
\begin{abstract}
We describe a finite, checkable reduction from 110 explicitly enumerated restricted tensor-rank lower bounds to a lower bound of 21 for the exact bilinear complexity of $3\times3$ matrix multiplication over $\mathbb F_2$. Restriction inequalities first force the first-factor forms in a hypothetical 20-term decomposition to be distinct and singular. The remaining cases are excluded using two direct restrictions and seven exact counting certificates, comprising 9,683 nodes and 4,845 leaves. A portable Python checker rederives every retained counting inequality, checks physical tensor symmetries, enumerates all 43,071 unordered rank-two pairs, and verifies exact branch and rational certificates. Crucially, that checker \emph{assumes} the 110 restricted-rank premises: it does not replay their original proofs. The conditional theorem established by the portable artifact is distinguished throughout from the broader local campaign's reported complete proof chain. This document is a review draft, not a claim of external acceptance or a novelty determination.
\end{abstract}
...
The portable checker verifies the implication in Theorem~\ref{thm:main}. It does not certify Assumption~\ref{ass:premises}. Of its 110 listed bounds, 105 are no stronger than the pinned published catalog bounds; five exceed that pinned baseline, at indices $70,423,486,488,494$. Both classes remain explicit assumptions of the portable replay. Publication of a premise elsewhere and local records of its verification are evidence about that premise, not an execution of its proof inside this checker.

The larger local campaign reports a completed chain of published-certificate replays, recovered restricted certificates, and global case checks supporting an unconditional claim. The frozen campaign record is broader than the portable reduction. A reviewer wishing to validate that unconditional claim must also inspect and independently replay the omitted restricted-bound chain. This draft does not equate a successful portable run with that stronger task, nor does it claim external peer review.
...
The default portable reduction contains a standard-library Python checker, a compressed JSON payload, a readable premise list, source provenance and integrity manifests. Its replay requires no LP solver, custom native binary, network access, or original campaign path. From the repository root, run:
\begin{verbatim}
python3 verification/verify.py
\end{verbatim}
Python 3.10 or newer is required. The checker also runs when invoked by an absolute path from another working directory. Python's assertion-disabling \texttt{-O} mode is rejected.

A successful default run explicitly reports
\begin{quote}\small\ttfamily
lower\_bound\_premises\_replayed: false
\end{quote}
This is part of the intended contract, not a missing success flag. Additional entry points are:
\begin{verbatim}
...
\end{verbatim}
The published mode replays the 496-entry pinned published catalog; that separate mode requires a C++ compiler and its bundled certificate archive. It does not establish the five strengthened premises in this draft. The repository document \path{docs/VERIFICATION.md} describes the mode-specific requirements and remaining strengthened-premise integration boundary. Appendix~\ref{app:premises} and the readable \path{PREMISES.json} enumerate the assumptions. Hashes authenticate a particular artifact snapshot; they are not mathematical evidence of a lower bound by themselves.
...
The principal unresolved task \emph{for a reviewer of the portable package} is to verify all restricted-rank premises from their original certificates and recovered dependencies. The larger campaign reports doing so locally; that report must not be substituted for the omitted replay. Review should also scrutinize the dual-coordinate convention, physical transpose action, direct restrictions, exact leaf rules, and the distinction in Remark~\ref{rem:subset}. The project has not established priority against all unpublished work, secured external acceptance, or demonstrated prize eligibility or commercial value.

\paragraph{AI-assistance disclosure.}
OpenAI Codex agents substantially assisted literature lookup, programming, certificate generation, mathematical exposition, and internal review in this project. Separate agents performed some separately implemented checks. Their organizational separation does not imply statistical independence, external expertise, or a replacement for human mathematical review. The author field is deliberately neutral pending confirmation of human authorship, responsibility, and affiliations. No affiliation or external endorsement is asserted.
